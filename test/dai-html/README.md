...
4. **WordPressテーマの場合**:
   - `dai-runner/config.js` の `dev.mode` を `'proxy'` に変更
   - `dev.proxy.proxyReq` 内の `Host` ヘッダーを開発環境のドメインに変更
...
├── dai-runner/               # ビルドシステム
│   ├── scripts/              # メインスクリプト（dev.js, build.js等）
│   ├── tasks/                # タスク定義（CSS, JS, 画像処理等）
│   ├── utils/                # ユーティリティ関数
│   ├── config.js             # ビルド設定
│   ├── config.js.example     # 設定ファイルのサンプル
│   └── README.md             # ビルドシステムのドキュメント
├── includes/                 # PHP機能ファイル
│   ├── post-type/            # カスタム投稿タイプ
