...
- SCSS 監視のイベント処理をスケジューラ（`tasks/css/scssScheduler.js`）に集約。ブランチ切替や一括生成のようにパーシャルの追加・変更・削除がまとめて届いた時、`_index.scss`
  の再生成を 1 回、メインファイルのコンパイルを 1 回にまとめる（以前は追加・削除のたびに再生成し、300ms 後にコンパイルを繰り返していた）。書き込みが途切れない場合でも最初の依頼から 2 秒で一度は走らせる
- SCSS のコンパイルを同時に 1 本だけ走らせる。コンパイル中に届いた変更は次の 1 回に合流させ、同じ CSS を複数のコンパイルが同時に書くことを無くした。最後に走る 1 回が必ず最新のソースを反映する
- メインファイル（`style.scss`
  など）の直接編集もパーシャルと同じ待ち（300ms）を経て走るようにし、連続保存を 1 回にまとめる
- 監視側が `_index.scss`
  を書き直した直後に返ってくる自分自身の追加・変更イベントは無視する（内容は直後のコンパイルに含まれているため、2 周目のコンパイルが無くなる）

### 内部

- `initScss` が実際に書き込んだ `_index.scss`
  のパスを返すようにした（監視側が自分の書き込みを見分けるため）
- スケジューラの単体テストを追加（バーストの集約・実行中の合流・maxWait・削除済みメインの除外・例外後の継続）
